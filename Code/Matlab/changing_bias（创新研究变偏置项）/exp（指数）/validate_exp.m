...
for i = 1:num_qr
    ...
    [results_cbpng(i).terminal_angle, results_cbpng(i).end_distance, results_cbpng(i).tt,...
     results_cbpng(i).theta_values, results_cbpng(i).a_M_end] = simulate_fixed_b(b_theory, N, v, r0, q0, theta0);
    
    % 情况2：神经网络定b（固定b_pred_nn）
    [results_nncbpng(i).terminal_angle, results_nncbpng(i).end_distance, results_nncbpng(i).tt,...
     results_nncbpng(i).theta_values, results_nncbpng(i).a_M_end] = simulate_fixed_b(b_pred_nn, N, v, r0, q0, theta0);
    
    % 情况3：变b模型（动态调整b_pred_exp）
    [results_adaptive(i).terminal_angle, results_adaptive(i).end_distance, results_adaptive(i).tt,...
     results_adaptive(i).theta_values, results_adaptive(i).a_M_end] = simulate_adaptive_b(b_pred_exp, N, v, r0, q0, theta0, trigger_distance, k);
end
...
function [terminal_angle, miss_distance, tt, theta_values, n_end] = simulate_fixed_b(b, N, v, r0, q0, theta0)
    ...
    theta_values = y(:,3);
    a_M_end = y(end,6);  % 末端加速度
    n_end = a_M_end / 9.81; % 转换为过载(g)
